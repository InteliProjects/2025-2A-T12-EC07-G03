...
export const LoginForm: React.FC<LoginFormProps> = ({ onSubmit, onRegister }) => {
  ...
  return (
    <div className="container max-w-lg mx-0 ml-16 px-0 pt-32 max-lg:ml-12 max-lg:pt-28 max-md:ml-8 max-md:pt-24 max-sm:ml-6 max-sm:pt-20">
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5 max-w-md">
        ...
        <div className="flex justify-between items-center mb-6 max-md:mb-5 max-sm:mb-4">
          <div className="flex items-center gap-3">
            <input
              id="remember"
              type="checkbox"
              {...register('remember')}
              className="w-4 h-4 border border-neutral-200 bg-white border-solid max-sm:w-3 max-sm:h-3"
            />
            <label 
              htmlFor="remember"
              className="text-[rgba(0,0,0,0.61)] text-sm font-normal max-sm:text-xs cursor-pointer"
            >
              Lembrar
            </label>
          </div>
          <button
            ...
            className="text-[rgba(0,0,0,0.61)] text-sm font-normal max-sm:text-xs hover:text-[#1934A5] transition-colors"
          >
            Esqueceu sua senha?
          </button>
        </div>
